...
@app.route("/api/dump")
def api_dump():
    """Dump data from OLD expired database — use OLD_DATABASE_URL env var."""
    old_db = os.environ.get("OLD_DATABASE_URL", "")
    if not old_db:
        return """
        <html>
        <body style="font-family:monospace;background:#0a0a0a;color:#fff;padding:40px;">
            <h2>❌ OLD_DATABASE_URL not set</h2>
            <p>Go to Render → Environment → add OLD_DATABASE_URL with your expired database connection string.</p>
            <p>Find it in Render → agent-db-2 → Connect button → copy the connection string.</p>
        </body>
        </html>
        """, 400
    try:
        ...
        try:
            cur.execute("SELECT * FROM do_not_repeat")
            dnr = [dict(r) for r in cur.fetchall()]
        except:
            dnr = []
        ...
        return Response(
            json.dumps(export_data, indent=2, default=str),
            mimetype="application/json",
            headers={"Content-Disposition": "attachment; filename=full_dump.json"}
        )
    except Exception as e:
        return f"""
        <html>
        <body style="font-family:monospace;background:#0a0a0a;color:#fff;padding:40px;">
            <h2>❌ Dump failed</h2>
            <p>Error: {str(e)}</p>
            <p>Make sure OLD_DATABASE_URL is correct and the database is still accessible.</p>
        </body>
        </html>
        """, 500
